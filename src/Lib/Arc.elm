...
arcSegment args =
    ...
    in
        [ if args.absolute then
            ...
            "a"
          -- 'A' for arc. Capital for absolute, lower for relative - isn't it obvious?!
        , toString radiusX
          -- radius X
        , toString radiusY
          -- radius Y
        , toString args.xAxisRotation
          -- x axis rotation
...
getArcInfo args =
    let
        ...
        toAbsoluteCoords angleDegrees =
            let
                ( normPointX, normPointY ) =
                    fromPolar ( 1.0, fromDegrees angleDegrees )

                pointX =
                    centerX + (normPointX * radius)

                pointY =
                    centerY - (normPointY * radius)
            in
                ( pointX, pointY )

        startPoint =
            toAbsoluteCoords args.startAngle

        endPoint =
            toAbsoluteCoords args.endAngle
    in
...
arc args =
    let
        ...
        largeArc =
            if args.endAngle - args.startAngle >= 180.0 then
                True
            else
                False
    in
        ("M "
            ++ (toString startPointX)
            ++ " "
            ++ (toString startPointY)
            ++ " "
            ++ (arcSegment
                    { absolute = True
