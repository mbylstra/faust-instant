...
type alias Model =
  { keysDown : Array Int
  , tempo : Float
  , currPosition : Int -- A number from 0 - 15 (16 quarter notes)
  }


getChord : Model -> (Note, Note, Note)
getChord model =
  (C, Ds, E)  -- assumes minor triad for now


getCurrentNote : Model -> Int
getCurrentNote model =
  let
    (triad1, triad2, triad3) = getChord model
    (octave, triadNum) = Array.get model.currPosition patternTable |> unsafeMaybe
    note =
      case triadNum of
        1 -> triad1
        2 -> triad2
        3 -> triad3
        _ -> Debug.crash "unexpected triad number"
    midiNote = 48 + (octave * 12) + (noteToInt note)
  in
    midiNote
...
init : Model
init =
  { keysDown = Array.fromList [60, 64, 68]
  -- , tempo = 120.0
  , tempo = 480.0
  , currPosition = 0
  }
...
update action model =
  case action of
    Tick ->
      let
        currPosition = (model.currPosition + 1) % 16
        model = { model | currPosition = currPosition }
      in
